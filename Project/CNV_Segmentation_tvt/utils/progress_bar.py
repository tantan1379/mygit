...
class Train_ProgressBar(object):
    ...
    def __init__(self, mode='train', epoch=None, total_epoch=None, current_loss=None, current_lr=0, model_name=None, total=None, current=None, width=80, symbol=">", output=sys.stderr):
        assert len(symbol) == 1
...
class Val_ProgressBar(object):
    def __init__(self, mode='val', model_name=None, total=None, current=None, width=29, symbol=">", output=sys.stderr):
        assert len(symbol) == 1
        self.mode = mode  # 文字显示模式
        self.total = total  # batch数量
        self.symbol = symbol  # bar显示符号，默认为<
...
class Test_ProgressBar(object):
    def __init__(self, mode='test', total=None,model_name=None,current=None, width=29, symbol=">", output=sys.stderr):
        assert len(symbol) == 1
        self.mode = mode  # 文字显示模式
        self.total = total  # batch数量
        self.symbol = symbol  # bar显示符号，默认为<
    ...
    def done(self):
        ...
        print("")
        # 向logs输出当前的结果
        with open("./logs/%s_best_indicator.txt" % (self.model_name), "a") as f:
