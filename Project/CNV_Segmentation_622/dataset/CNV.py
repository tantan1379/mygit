...
class CNV(data.Dataset):
    ...
    def read_list(self, image_path):
        ...
        label_list = list()
        if self.mode == 'train':
            img_list += glob.glob(os.path.join(image_path, "train")+'/*.png')
            label_list = [x.replace('img', 'mask') for x in img_list]
        elif self.mode == 'val':
            img_list += glob.glob(os.path.join(image_path, "val")+'/*.png')
            label_list = [x.replace('img', 'mask') for x in img_list]
        elif self.mode == 'test':
            img_list += glob.glob(os.path.join(image_path, "test")+'/*.png')
            label_list = [x.replace('img', 'mask') for x in img_list]
        return img_list, label_list


if __name__ == '__main__':
    dataset_path = r"F:\Dataset\CNV_Seg\png_split_622"
    dataset = CNV(dataset_path, scale=(512, 256),mode='val')
    # print(len(dataset))
    dataset[0]
