...
def val(args, model, dataloader, epoch):
    with torch.no_grad():
        ...
        eval_progressor = pb.Val_ProgressBar(
            mode='val', epoch=epoch+1, model_name=args.net_work, net_index=args.net_index, total=len(dataloader))
...
def train(args, model, optimizer, criterion, dataloader_train, dataloader_val, writer):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (data, label) in enumerate(dataloader_train):
            ...
            if step % 10 == 0:
                writer.add_scalar(
                    'Train/loss_step', loss, step)
        train_progressor.done()
        writer.add_scalar(
            'Train/loss_epoch', float(train_loss.avg), epoch)
        Dice, Acc, jaccard, Sensitivity, Specificity = val(
            args, model, dataloader_val, epoch)
        writer.add_scalar(
            'Valid/Dice_val', Dice, epoch)
        writer.add_scalar(
            'Valid/Acc_val', Acc, epoch)
        writer.add_scalar(
            'Valid/Jac_val', jaccard, epoch)
        writer.add_scalar(
            'Valid/Sen_val', Sensitivity, epoch)
        writer.add_scalar(
            'Valid/Spe_val', Specificity, epoch)
...
def main(mode='train', args=None, writer=None):
    ...
    model_all = {'UNet': UNet(in_channels=args.input_channel, n_classes=args.num_classes),
                 ...
                 'cpfnet':CPFNet(out_planes=args.input_channel)
                 }
    ...
    if mode == 'train':  # 交叉验证
        train(args, model, optimizer, criterion,
              dataloader_train, dataloader_val, writer)
